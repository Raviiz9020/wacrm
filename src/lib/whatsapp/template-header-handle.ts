...
export async function ensureImageHeaderHandle(
  ...
  const appId = (explicitAppId && explicitAppId.trim()) || process.env.META_APP_ID
  if (!appId) {
    throw new Error(
      'Image-header templates need a Meta App ID (configured in Settings → WhatsApp, or set META_APP_ID in environment).',
    )
  }
